...
export default async function GameDetailPage({ params }) {
  ...
  const currentGame = detail.game
  const prevGameId = detail.prevGameId
  const nextGameId = detail.nextGameId
  return (
    <main className="w-full bg-white pb-16 min-h-screen">
      ...
      />
      <ReviewsSection
        gameSlug={currentGame.id}
        gameTitle={currentGame.title}
      />
      <RelatedGames slug={currentGame.id} />
    </main>
  )
}
